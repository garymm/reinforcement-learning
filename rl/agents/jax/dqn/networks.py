...
def _conv_output_shape(
    ...
    for i, input_size in enumerate(input_shape):
        ...
        if input_size < filter_size:
            raise ValueError(
                f"Input size {input_size} in dimension {i} is less than filter size {filter_size}"
            )
...
class _QNetwork(eqx.Module):
    ...
    def __init__(self, spec: specs.EnvironmentSpec, key: jax_types.PRNGKey):
        ...
        if obs_rank != 4:
            raise ValueError(
                "Expected obs format (Stack, Height, Width, Channel). Got rank %d"
                % obs_rank
            )
        ...
        need_resize = False
        if (obs_shape[1], obs_shape[2]) != resize_image_to:
            need_resize = True
        ...
        layers: list[abc.Callable] = [
            # flatten frame stack
            eqx.nn.Lambda(lambda x: jnp.reshape(x, (-1,) + obs_shape[2:])),
    ...
    def __call__(self, x: jaxtyping.Array | np.ndarray) -> jaxtyping.Array:
        if x.ndim != 4:
            raise ValueError("Expected input to have rank 4. Got rank %d" % x.ndim)
