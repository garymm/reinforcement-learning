...
from rl.utils.wrappers import video
...
def run_experiment(
    ...
    actor = _LearningActor(
        actor, learner, dataset, replay_tables, rate_limiters_max_diff, checkpoint_dir
    )
    ...
    if num_eval_episodes == 0:
        ...
        train_loop.run(num_steps=int(max_num_actor_steps))
        return learner
    ...
    if eval_video_dir:
        eval_environment = video.VideoWrapper(
            eval_environment, path=eval_video_dir, process_path=os.path.join
        )
    ...
    while steps < max_num_actor_steps:
        ...
        steps += train_loop.run(num_steps=int(num_steps))
    eval_loop.run(num_episodes=num_eval_episodes)
...
class _LearningActor(core.Actor):
    ...
    def __init__(
        ...
        Args:
          ...
          checkpoint_dir: Where to save the state on update.
        """
        self._actor = actor
    ...
    def update(
        ...
        if self._maybe_train():
            ...
            self._actor.update()
        if self._checkpoint_dir and self._learner_steps % 1_000 == 0:
            learner_state = self._learner.save()
            filename = f"{self._checkpoint_dir}/model_state_{self._learner_steps}.pkl"
            with open(filename, "wb") as f:
                f.write(pickle.dumps(learner_state))
